...
const post_table_view = async (req: Request) => {
  const url = new URL(req.url);
  if (url.pathname === "/api/ledger" && req.method === "POST") {
    try {
      const { data, error } = await supabase.handlePost(req, "ledger");
  ...
  if (url.pathname === "/api/payment" && req.method === "POST") {
    try {
      const { data, error } = await supabase.handlePost(req, "payment");
      console.log("data check in view", data);
...
export default post_table_view;
